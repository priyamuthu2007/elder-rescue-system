...
- A feed of every report — including a photo thumbnail if one was submitted — each with a visual progress tracker (Reported → Acknowledged → En route → Rescued), a "Close" option, and a one-click button to advance it to the next stage with an optional note
- A "View history" toggle per report showing the full timeline of status changes
- A sidebar to add test organizations and submit test reports (with optional photo upload) directly from the browser, instead of using curl/PowerShell
- Auto-refreshes every 6 seconds
- A link to open the public reporting page in a new tab, for quick testing

Neither page has login/auth yet — both are meant to be used by you locally for now, not deployed publicly as-is.
