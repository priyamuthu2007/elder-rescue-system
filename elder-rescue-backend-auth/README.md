...
## Four pages
- **`report.html`** — the public reporting page (unchanged)
- **`index.html`** — the network overview: shows *every* report across all organizations, useful for you as the admin/tester, or a bird's-eye view
- **`ngo-login.html`** — an organization picks itself from the list and enters its password to sign in
- **`ngo-dashboard.html`** — shows *only* the reports assigned to the signed-in organization, with the same status-update actions as before

## Organization passwords
Every organization now sets a password (minimum 6 characters) when it's
registered via the "Add organization" form. Passwords are hashed with
Node's built-in `crypto.scrypt` (`auth.js`) — never stored in plain
text, and never included in any API response (`GET /organizations`
strictly excludes it). Signing in on `ngo-login.html` calls
`POST /organizations/login` with `{ id, password }`, which the server
verifies with a timing-safe comparison before returning that org's
info.

**What this covers:** a real password is now required to access any
organization's dashboard — this is genuine protection, not just a
name-picker. **What it doesn't cover yet:** there's no session
expiry, no rate-limiting on login attempts, and no password-reset
flow — reasonable next steps before this handles real, sensitive
organizations at scale, but the core protection (you can't act on an
org's reports without its password) is real and tested.
