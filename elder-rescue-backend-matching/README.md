...
- `server.js` — Express API with endpoints to create/list reports and organizations, with automatic nearest-org matching
...
## Verified working
All endpoints and the matching logic were tested end-to-end before
delivery, including: nearest-org selection among multiple candidates,
correct exclusion of unverified orgs, correct exclusion of orgs whose
coverage radius doesn't reach the report, and the no-coverage fallback
case.

## Next steps (not built yet)
1. OTP phone verification before a report can be submitted
2. Status pipeline: reported → acknowledged → en_route → rescued/closed
3. NGO dashboard (simple web UI reading from GET /reports)
