...
- **`index.html`** — the network overview (renamed in purpose): shows *every* report across all organizations, useful for you as the admin/tester, or a bird's-eye view
- **`ngo-login.html`** — new: an organization selects itself from the list of registered orgs to "sign in"
- **`ngo-dashboard.html`** — new: shows *only* the reports assigned to the signed-in organization, with the same status-update actions as before

**Important honest caveat:** the NGO "sign-in" is a stand-in, not real authentication. An organization currently signs in by picking its own name from the list of registered orgs — there's no password, so right now anyone could select any organization's name and see/act on its reports. This is enough to demo and test the *separation* (each org's dashboard is now genuinely filtered to just its own reports), but real security (a password, or a proper login system) is still on the to-do list before this could be used with real, unrelated organizations.
...
1. **Real NGO authentication** — the current org "sign-in" is just picking a name from a list, not a real login; needs a password or similar before it's trustworthy
